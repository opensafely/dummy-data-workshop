...
from analysis.supporting_data.icd10_codes import HEART_FAILURE_CODES, NON_HEART_FAILURE_CODES
...
non_heart_failure_death = (
    ons_deaths.underlying_cause_of_death.is_in(NON_HEART_FAILURE_CODES)
    & ons_deaths.date.is_on_or_between(dummy_data_start_date, dummy_data_end_date)
)
...
hospital_admission_date = (
    apcs
    .where(
        apcs.admission_date.is_on_or_before(ons_deaths.date)
    )
...
dataset.define_population(registration.start_date.is_not_null() | non_heart_failure_death | heart_failure_death)
